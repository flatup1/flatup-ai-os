...
1. **設計方針は採用**。ComfyUI公式 I2Vテンプレ + Comfy-Org/MiniMax-H3 + Drive保存 + シンボリックリンクで正しい。実装済み → [`notebooks/colab_minimax_h3_i2v.ipynb`](../notebooks/colab_minimax_h3_i2v.ipynb)
...
| Drive保存＋シンボリックリンク | 採用 | **△ 条件つき** | **無料Drive 15GBでは42GBが入らない**。100GB以上のプランのときだけ有効 |
...
| ③ | ComfyUI導入 + cloudflared + モデルフォルダをリンク | 12-2・12-4 |
...
ノート⑦に **6場面** を実装済み。`SCENE` を書き換えるだけで完成形が出ます。

| キー | 場面 |
|---|---|
| **`first_punch`** | **はじめの一歩（緊張→笑顔→やさしいストレート）＝ メモ13・14の再現。既定値** |
| `wave` | おいでおいで（体験募集リールの冒頭） |
| `jab` | 左ジャブ1発 |
| `high_kick` | 元気なハイキック |
| `rest` | 休憩中（癒し系） |
| `coach_welcome` | 先生のごあいさつ |
| `mitt_night` | 夜のジムでミットくんが目を覚ます |
