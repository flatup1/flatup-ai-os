...
- プロンプトは技(モーション)中心に。被写体・背景は起点画像が決めるので、
  「1技だけ・カメラ固定・崩れ防止」を明示すると崩れにくい。
- 名鑑には **I2V 最適化済みの「ワンドクシング」(大型犬×ボクシング)** を登録済み。
  `npm run reel -- "ワンドクシング" --count 5 --stitch --image 犬.png` で、ジャブ/ワンツー/
  キック/ガード/フットワークの技違いを起点画像から生成→1本のリールに連結まで一発。
  ※ シリーズに `i2vSequences` があると、I2V モードでは動き中心の完結プロンプトを自動採用する。
