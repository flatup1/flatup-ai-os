...
| 項目 | 実際のところ |
|---|---|
| Google AI Pro | **Colab Pro は含まれない**。Colab は別契約になる |
| 必要な契約 | **Colab Pro 以上**（有料。A100 を使うには計算ユニットの購入が要る） |
| 消費の目安 | A100 はおよそ 5.4 ユニット/時。100ユニットで A100 約18時間ぶん |

> 💰 **料金は変わる**ので、契約前に Colab の料金ページで**今の金額とユニット数を必ず確認する**。
> ここに書いた消費の目安も Google 側の変更で動く。支払いは JIN の承認後。
